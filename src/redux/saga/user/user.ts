...
export function* updateAvatarSaga({ payload }: any): any {
  const toastId = toast.loading('Đang tải ảnh lên');
  try {
    const response = yield call(services.updateAvatarServices, payload);
    if (response.status === 200) {
      yield put(authRefreshUser());
    ...
    toast.update(toastId, {
      isLoading: false,
      type: 'error',
      render: 'Cập nhật ảnh đại diện thất bại',
      autoClose: 500,
    });
    console.error('Lỗi', err);
  }
